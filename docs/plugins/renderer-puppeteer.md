...
在插件市场里点「安装」，其余步骤自动完成：装依赖 → `build` → `install:browser` 下一份无头浏览器
（`chrome-headless-shell`，约 100MB，落在主目录的 `cache/puppeteer` 下）。那一步会下上百兆，
故确认框里会先把要跑的步骤列出来，装依赖与装后步骤各留 15 分钟超时。

这三步由索引条目的 `setup.scripts` 声明（见[插件市场](/market)），不是内核对本插件的特例。
**手工放进 `plugins/` 的副本没有安装动作可挂**，那时到插件市场页该条目的「更多」菜单里点
「装依赖并编译」补跑同样三步，或自行在插件目录下依次执行。

装完须在插件页重载 —— 渲染器是在插件 `setup()` 里注册的。
